    ...
    fluidPage(
        ...
        column(width = 5,
            ...
            column(width = 12, 
                HTML(paste0(
                    '<hr style="width: 100%;border-top: 1px solid #ddd;" />',
                    "<div style='font-size:17px;line-height: 2em;color: black;'> Project ID: PRJNA662458 </div>",
                    ...
                    "<div style='font-size:17px;line-height: 2em;color: black;'> Reference: Li F, Hu Q, Chen F, et al. Transcriptome analysis reveals vernalization is independent of cold acclimation in Arabidopsis [J]. BMC Genomics, 2021, 22 (1): 462. </div>"
                )),
                downloadButton("download_PRJNA662458", label = "Download PRJNA662458_gene_conut_matrix.csv", style = "width:500px;margin-bottom: 10px;"),
                HTML('<hr style="width: 100%;border-top: 1px solid #ddd;" />')
            ),
        ),
        column(width = 6, offset = 1, style = "margin-bottom:20px",
            radioGroupButtons(inputId = "guide_methods",label = NULL, choices = c("RNA-seq Upstream Methods", "ChIP-seq Upstream Methods"), justified = TRUE, width = "100%", checkIcon = list(yes = icon("ok", lib = "glyphicon")))
        ),
...
output$download_PRJNA662458 <- downloadHandler(
    filename = function() {"PRJNA662458_gene_conut_matrix.csv" },
    content = function(file) {file.copy("template/PRJNA662458_gene_count_matrix.csv", file)}
)
    ...
    fluidPage(
        column(width = 12,
            HTML(paste0(
                ...
                "<div style='font-size:17px;line-height: 2em;color: black'> If you have any questions or suggestions during use, please leave a message on GitHub or contact 364832885@qq.com by email. </div>",
                '<hr style="width: 100%;border-top: 1px solid #ddd;" />'
            ))
        )
    )
